#!/usr/bin/env python3
"""
One-file solver for DD LAB / 3_in_1_.pkt

What it does:
1. Builds a tiny native helper locally (gcc) to decrypt the Packet Tracer .pkt
2. Extracts the internal XML
3. Locates the workstation artifact "Borring novel .txt"
4. Decodes the embedded Shakespeare-style program
5. Prints the final accepted flag
...
UNPKT_FAST_C = r"""#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <string.h>
#include <zlib.h>
...
static void xor16(uint8_t *out, const uint8_t *a, const uint8_t *b){ for(int i=0;i<16;i++) out[i]=a[i]^b[i]; }
static void left_shift_one(uint8_t *out, const uint8_t *in){
    uint8_t carry=0;
    for(int i=15;i>=0;i--){ uint8_t newv=(uint8_t)(in[i]<<1); out[i]=newv|carry; carry=(in[i]&0x80)?1:0; }
}
static void enc_block(Twofish_key *xkey, const uint8_t in[16], uint8_t out[16]){
    uint8_t tmp[16]; memcpy(tmp,in,16); Twofish_encrypt(xkey,tmp,out);
}
static void cmac_subkeys(Twofish_key *xkey, uint8_t K1[16], uint8_t K2[16]){
    uint8_t zero[16]={0}, L[16], rb[16]={0}; rb[15]=0x87;
    enc_block(xkey,zero,L); left_shift_one(K1,L); if(L[0]&0x80){ for(int i=0;i<16;i++) K1[i]^=rb[i]; }
    left_shift_one(K2,K1); if(K1[0]&0x80){ for(int i=0;i<16;i++) K2[i]^=rb[i]; }
}
static void pad_block(uint8_t out[16], const uint8_t *block, size_t len){
    memset(out,0,16); if(len) memcpy(out,block,len); out[len]=0x80;
}
static void cmac_digest(Twofish_key *xkey, const uint8_t *data, size_t len, uint8_t out[16]){
    uint8_t K1[16],K2[16],X[16]={0},Y[16],last[16]; cmac_subkeys(xkey,K1,K2);
    size_t n = (len + 15) / 16;
    int complete = (len>0 && (len%16)==0);
    if(n==0) n=1;
    size_t blocks_before_last = n-1;
    for(size_t bi=0; bi<blocks_before_last; bi++){
        for(int i=0;i<16;i++) Y[i]=X[i]^data[bi*16+i];
        enc_block(xkey,Y,X);
    }
    if(complete){
        const uint8_t *last_in = data + (n-1)*16;
        for(int i=0;i<16;i++) last[i] = last_in[i] ^ K1[i];
    } else {
        size_t rem = (len==0) ? 0 : (len - (n-1)*16);
        uint8_t padded[16]; pad_block(padded, data + (n-1)*16, rem);
        for(int i=0;i<16;i++) last[i] = padded[i] ^ K2[i];
    }
    for(int i=0;i<16;i++) Y[i]=X[i]^last[i];
    enc_block(xkey,Y,out);
}
static void omac_prefix(Twofish_key *xkey, int prefix, const uint8_t *data, size_t len, uint8_t out[16]){
    uint8_t *buf = malloc(16 + len); if(!buf){fprintf(stderr,"malloc omac\n"); exit(1);} 
    memset(buf,0,16); buf[15]=(uint8_t)prefix; if(len) memcpy(buf+16,data,len);
    cmac_digest(xkey,buf,16+len,out); free(buf);
}
static void inc_be(uint8_t c[16]){ for(int i=15;i>=0;i--){ c[i]=(uint8_t)(c[i]+1); if(c[i]!=0) break; } }
static void ctr_process(Twofish_key *xkey, const uint8_t init[16], const uint8_t *in, size_t len, uint8_t *out){
    uint8_t counter[16],ks[16]; memcpy(counter,init,16);
    size_t off=0; while(off<len){ enc_block(xkey,counter,ks); inc_be(counter); size_t bl = len-off<16?len-off:16; for(size_t i=0;i<bl;i++) out[off+i]=in[off+i]^ks[i]; off += bl; }
}
static void eax_decrypt(Twofish_key *xkey, const uint8_t nonce[16], const uint8_t *ct, size_t ctlen, const uint8_t tag[16], uint8_t *pt){
    uint8_t ntag[16],htag[16],ctag[16],tmp[16],exp[16];
    omac_prefix(xkey,0,nonce,16,ntag);
    ctr_process(xkey,ntag,ct,ctlen,pt);
    omac_prefix(xkey,1,NULL,0,htag);
    omac_prefix(xkey,2,ct,ctlen,ctag);
    xor16(tmp,ntag,htag); xor16(exp,tmp,ctag);
    if(memcmp(exp,tag,16)!=0){
        fprintf(stderr,"EAX auth failed\nexpected: "); for(int i=0;i<16;i++) fprintf(stderr,"%02x",exp[i]);
        fprintf(stderr,"\ntag:      "); for(int i=0;i<16;i++) fprintf(stderr,"%02x",tag[i]); fprintf(stderr,"\n"); exit(2);
    }
}
static uint8_t *read_file(const char *path, size_t *len){
    FILE *f=fopen(path,"rb"); if(!f){perror("fopen"); exit(1);} fseek(f,0,SEEK_END); long l=ftell(f); fseek(f,0,SEEK_SET); uint8_t *b=malloc(l); if(!b){fprintf(stderr,"malloc file\n"); exit(1);} if(fread(b,1,l,f)!=(size_t)l){fprintf(stderr,"read fail\n"); exit(1);} fclose(f); *len=l; return b;
}
static void write_file(const char *path, const uint8_t *data, size_t len){ FILE *f=fopen(path,"wb"); if(!f){perror("fopen out"); exit(1);} fwrite(data,1,len,f); fclose(f); }
int main(int argc, char **argv){
    const char *in = argc>1 ? argv[1] : "/mnt/data/3_in_1_ (1).pkt";
    const char *outpath = argc>2 ? argv[2] : "/mnt/data/3_in_1.xml";
    size_t L; uint8_t *pkt=read_file(in,&L);
    uint8_t *stage1=malloc(L); if(!stage1){fprintf(stderr,"malloc stage1\n"); return 1;}
    for(size_t i=0;i<L;i++) stage1[i] = pkt[L-1-i] ^ ((L - i*L) & 0xff);
    if(L<16){fprintf(stderr,"too small\n"); return 1;}
    size_t ctlen=L-16; uint8_t *dec=malloc(ctlen); uint8_t key[16], nonce[16]; for(int i=0;i<16;i++){key[i]=137; nonce[i]=16;}
    Twofish_key xkey; Twofish_initialise(); Twofish_prepare_key(key,16,&xkey);
    eax_decrypt(&xkey,nonce,stage1,ctlen,stage1+ctlen,dec);
    uint8_t *stage2=malloc(ctlen); for(size_t i=0;i<ctlen;i++) stage2[i]=dec[i]^((ctlen-i)&0xff);
    uint32_t outlen = ((uint32_t)stage2[0]<<24)|((uint32_t)stage2[1]<<16)|((uint32_t)stage2[2]<<8)|stage2[3];
    uint8_t *xml=malloc(outlen+1); if(!xml){fprintf(stderr,"malloc xml %u\n",outlen); return 1;} uLongf dest=outlen;
    int zr=uncompress(xml,&dest,stage2+4,ctlen-4); if(zr!=Z_OK){fprintf(stderr,"zlib fail %d outlen %u dest %lu\n",zr,outlen,(unsigned long)dest); return 1;}
    xml[dest]=0; write_file(outpath,xml,dest); printf("wrote %s len %lu\n",outpath,(unsigned long)dest);
    return 0;
}
...
def build_helper(workdir: Path) -> Path:
    gcc = shutil.which("gcc")
    if not gcc:
        raise SystemExit(
            "gcc was not found. Install a compiler first.\n"
            "Debian/Ubuntu/Kali: sudo apt install build-essential zlib1g-dev"
        )

    (workdir / "twofish.h").write_text(TWOFISH_H, encoding="utf-8")
    (workdir / "twofish.c").write_text(TWOFISH_C, encoding="utf-8")
    (workdir / "unpkt_fast.c").write_text(UNPKT_FAST_C, encoding="utf-8")

    out = workdir / "unpkt_fast"
    cmd = [
        gcc,
        "-O2",
        "-std=c99",
        str(workdir / "unpkt_fast.c"),
        str(workdir / "twofish.c"),
        "-lz",
        "-o",
        str(out),
    ]
...
def decrypt_pkt_to_xml(pkt_path: Path, workdir: Path) -> str:
    helper = build_helper(workdir)
    xml_path = workdir / "out.xml"

    try:
        res = subprocess.run(
            [str(helper), str(pkt_path), str(xml_path)],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    if not xml_path.exists():
        raise SystemExit(
            "Decrypt helper reported success but XML was not written.\n"
            f"helper stdout:\n{res.stdout}\nhelper stderr:\n{res.stderr}"
        )
...
def extract_text_file(xml: str, filename_regex: str) -> str | None:
    pattern = (
        r'<FILE class="CFile">.*?'
        r'<NAME>(' + filename_regex + r')</NAME>.*?'
        r'<TEXT>(.*?)</TEXT>.*?'
        r'</FILE>'
    )
...
def decode_spl_output(src: str) -> str:
    """
    Minimal Shakespeare Programming Language decoder for this specific lab.

    The embedded "Borring novel .txt" uses a tiny subset:
      - 'You are ...' assigns Juliet
      - 'Speak your mind!' outputs Juliet as ASCII
      - 'flower' = 1
      - each 'sweet' doubles the magnitude
      - 'yourself' refers to the current Juliet value
    """
    juliet = 0
    out = []

    def value_phrase(p: str) -> int:
        nonlocal juliet
        p = p.strip().strip(".")
        if p == "yourself":
            return juliet
        if "flower" in p:
            sweet_count = len(re.findall(r"\bsweet\b", p))
            return 2 ** sweet_count
        raise ValueError(f"unknown SPL phrase: {p!r}")

    def eval_expr(expr: str) -> int:
        expr = expr.strip().strip(".")
        expr = re.sub(r"^as sweet as the sum of\s+", "", expr)
        expr = re.sub(r"^the sum of\s+", "", expr)
        expr = re.sub(r"^as sweet as\s+", "", expr)
        parts = [x.strip() for x in re.split(r"\s+and\s+", expr) if x.strip()]
        return sum(value_phrase(p) for p in parts)

    for line in src.splitlines():
        line = line.strip()
        if line.startswith("You are "):
            juliet = eval_expr(line[len("You are "):])
        elif line.startswith("Speak your mind"):
            out.append(chr(juliet & 0xFF))

    return "".join(out)


def extract_recovery_secret(xml: str) -> str | None:
    m = re.search(r"(ZDIwX1NhbHRvTmF6YWQ=)", xml)
    if not m:
        return None
...
def main():
    if len(sys.argv) != 2:
        raise SystemExit(f"Usage: python3 {Path(sys.argv[0]).name} 3_in_1_.pkt")

    pkt_path = Path(sys.argv[1])
    if not pkt_path.exists():
        raise SystemExit(f"Input file not found: {pkt_path}")

    print("[*] Decrypting Packet Tracer project...")
    with tempfile.TemporaryDirectory(prefix="solve_3in1_") as td:
        xml = decrypt_pkt_to_xml(pkt_path, Path(td))

    if "<PACKETTRACER" not in xml:
        raise SystemExit("Decrypted output does not look like Packet Tracer XML")

    print("[*] Extracting recovery/access secret from port/config side...")
    recovery = extract_recovery_secret(xml)
    if recovery:
        print(f"    {recovery}")
    else:
        print("    not found")
    ...
    novel = extract_text_file(xml, r"Borring novel \.txt")
    if not novel:
        raise SystemExit("Could not locate 'Borring novel .txt' in decrypted XML")

    decoded = decode_spl_output(novel)
    print(decoded)

    reward = decoded.strip().splitlines()[-1].strip()

    print("\n[+] Final flag:")
    print(f"KubSTU({reward})")
